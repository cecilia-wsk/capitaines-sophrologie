...
export default function Navbar({ visible }) {
  ...
  useEffect(() => {
    ...
    const items = wrap.querySelectorAll("a, button");

    gsap.killTweensOf([wrap, items]);
  ...
  /* Mobile menu: stagger the links in as the bar expands */
  useEffect(() => {
    if (!open) return;
    const links = menuRef.current.querySelectorAll(".mobile-nav__list a");
    gsap.fromTo(
      links,
      { autoAlpha: 0, y: 16 },
      {
  ...
  return (
    <>
      <div ref={wrapRef} className={`nav-wrap${open ? " menu-open" : ""}`}>
        {/* Desktop pill */}
        ...
        {/* Mobile bar: the rectangle expands to reveal the links */}
            ...
            >
              {open ? (
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="m5.99 6.697.707-.707L18.01 17.303l-.707.707L5.99 6.697ZM6.697 18.01l-.707-.707 4.596-4.596.707.707-4.596 4.596ZM13.414 11.293l-.707-.707 4.596-4.596.708.707-4.597 4.596Z" />
                </svg>
              ) : (
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M4 6.5h16v1H4v-1ZM4 11.5h16v1H4v-1ZM4 16.5h12v1H4v-1Z" />
                </svg>
              )}
            </button>
          </div>

          <div className="mobile-nav__links" ref={menuRef} id="mobile-menu">
            <div className="mobile-nav__clip">
              <nav className="mobile-nav__list" aria-label="Menu">
                {ALL_LINKS.map((link) => (
                  <a
                    key={link.href}
                    href={link.href}
